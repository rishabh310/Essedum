...
No design JSON found, using minimal workflow
No nodes found in design JSON, creating default simple workflow
Failed to create workflow: 'AppConfig' object has no attribute 'MODEL_NAME'
Failed to initialize workflow: 'AppConfig' object has no attribute 'MODEL_NAME'
ERROR:    Traceback (most recent call last):
  ...
  File "/usr/local/lib/python3.11/site-packages/starlette/routing.py", line 671, in startup
    await handler()
  File "/app/src/main.py", line 105, in startup_event
    workflow_instance = create_workflow(design_json, config_instance)
                        ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "/app/src/graph_builder.py", line 158, in create_workflow
    return create_default_workflow(config)
           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "/app/src/graph_builder.py", line 121, in create_default_workflow
    model_name=config.MODEL_NAME,
               ^^^^^^^^^^^^^^^^^
  File "/usr/local/lib/python3.11/site-packages/pydantic/main.py", line 1026, in __getattr__
    raise AttributeError(f'{type(self).__name__!r} object has no attribute {item!r}')
AttributeError: 'AppConfig' object has no attribute 'MODEL_NAME'
